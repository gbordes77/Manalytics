...
def main():
    ...
    # Créer le nom du dossier avec les dates
    min_date = analyzer.df['tournament_date'].min().strftime('%Y-%m-%d')
    max_date = analyzer.df['tournament_date'].max().strftime('%Y-%m-%d')
    output_dir = f"standard_analysis_{min_date}_{max_date}"
