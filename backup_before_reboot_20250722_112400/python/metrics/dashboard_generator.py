...
class DashboardGenerator:
    ...
    def generate_metagame_pie_chart(self, metagame_data: Dict) -> go.Figure:
        ...
        fig.update_layout(
            title="Répartition du Métagame", font=dict(size=12), showlegend=True
        )
    ...
    def generate_comprehensive_dashboard(self, analysis_data: Dict) -> str:
        ...
        if metagame_share:
            ...
            fig.add_trace(
                go.Pie(labels=labels, values=values, name="Métagame"), row=1, col=1
            )
        ...
        if win_rates:
            ...
            fig.add_trace(
                go.Bar(x=archetypes, y=winrates, name="Taux de Victoire"), row=1, col=2
            )
        ...
        fig.update_layout(
            height=800, title_text="Dashboard Métagame Complet", showlegend=False
        )
