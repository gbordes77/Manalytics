...
class DynamicRulesEngine:
    ...
    def start_watching(self, use_polling: bool = False):
        ...
        try:
            self.file_watcher = create_file_watcher(
                self.on_rules_change, use_polling=use_polling
            )
        ...
        with self._lock:
            try:
                ...
                if new_rules and new_rules.get("archetypes"):
                    # Update cache
                    old_count = len(
                        self.rules_cache.get(format_name, {}).get("archetypes", [])
                    )
                    ...
                    logger.info(
                        f"Reloaded {format_name}: {old_count} -> {new_count} archetypes"
                    )
                else:
    ...
    def classify_deck(self, deck: Dict[str, Any], format_name: str) -> Optional[str]:
        ...
        # Extract deck cards
        mainboard = {
            card.get("name", ""): card.get("count", 0)
        ...
        for archetype in archetypes:
            if self._matches_conditions(
                mainboard, sideboard, archetype.get("conditions", [])
            ):
    ...
    def _matches_condition(
        ...
        if condition_type == "contains":
            ...
            total = sum(
                mainboard.get(card, 0) + sideboard.get(card, 0) for card in cards
            )
