...
class RetryHandler:
    ...
    def __init__(
        ...
        logger.info(
            f"Retry Handler '{name}' initialisé: "
    ...
    async def call(self, func: Callable, *args, **kwargs) -> Any:
        ...
        for attempt in range(1, self.max_attempts + 1):
            try:
                ...
                if attempt > 1:
                    logger.info(
                        f"Retry Handler '{self.name}' succès à la tentative {attempt}"
                    )
                ...
                if attempt == self.max_attempts:
                    ...
                    logger.error(
                        f"Retry Handler '{self.name}' échec après {self.max_attempts} tentatives: {e}"
                    )
                    self.failed_calls += 1
                    break
                ...
                logger.warning(
                    f"Retry Handler '{self.name}' tentative {attempt} échouée: {e}. "
                ...
                logger.error(
                    f"Retry Handler '{self.name}' exception non-retryable: {e}"
                )
                self.failed_calls += 1
                raise e
        ...
        raise RetryExhaustedError(
            f"Retry épuisé après {self.max_attempts} tentatives. "
    ...
    def __repr__(self) -> str:
        return (
            f"RetryHandler(name='{self.name}', strategy={self.strategy.value}, "
