...
class ErrorMonitor:
    ...
    def __init__(
        ...
        self.events: Dict[str, deque] = defaultdict(
            lambda: deque(maxlen=max_events_per_component)
        )
    ...
    def _add_event(self, event: ErrorEvent):
        ...
        logger.debug(
            f"Error Monitor: {event.component} - {event.error_type}: {event.error_message}"
        )
    ...
    def _update_stats(self, component: str):
        ...
        error_rate = (
            len(recent_events) / self.time_window_minutes
    ...
    def _check_alert_conditions(self, component: str):
        ...
        if alert_key in self.suppressed_alerts:
            if (
                current_time - self.suppressed_alerts[alert_key]
                < self.alert_suppression_time
            ):
                return
        ...
        recent_events = [
            e for e in self.events[component] if current_time - e.timestamp < 60
        ]  # 1 minute
        if len(recent_events) >= self.burst_threshold:
            burst_key = f"{component}_burst"
            if (
                burst_key not in self.suppressed_alerts
                or current_time - self.suppressed_alerts[burst_key]
                >= self.alert_suppression_time
            ):
                self._trigger_alert(
    ...
    def _trigger_alert(
        self, alert_type: str, component: str, message: str, severity: str
    ):
    ...
    def get_error_summary(self) -> dict:
        ...
        active_components = sum(
            1
            for stats in self.components_stats.values()
            if stats.last_error_time and time.time() - stats.last_error_time < 3600
        )

        total_recent_errors = sum(
            len(stats.recent_errors) for stats in self.components_stats.values()
        )
        ...
        return {
            ...
            "components": {
                name: stats.total_errors
                for name, stats in self.components_stats.items()
            },
        }

    def export_errors(
        self, component: Optional[str] = None, format: str = "json"
    ) -> str:
