...
from .schema_versions import BaseSchema, MeleeSchemaV1, MTGOSchemaV1, MTGOSchemaV2
...
class SchemaManager:
    ...
    def detect_schema_version(
        ...
        try:
            ...
            if source:
                candidates = [
                    k for k in self.schemas.keys() if k.startswith(source.lower())
                ]
            else:
            ...
            if best_match and best_score > 0.5:  # Minimum confidence threshold
                logger.info(
                    f"Detected schema: {best_match} (confidence: {best_score:.2f})"
                )
                return best_match
    ...
    def _calculate_schema_score(self, data: Dict[str, Any], schema_name: str) -> float:
        ...
        if required_fields:
            required_score = sum(1 for field in required_fields if field in data) / len(
                required_fields
            )
        ...
        if signature_fields:
            signature_score = sum(
                1 for field in signature_fields if self._field_exists_deep(data, field)
        ...
        if unique_fields:
            unique_score = sum(
                1 for field in unique_fields if self._field_exists_deep(data, field)
            ) / len(unique_fields)
    ...
    def _fallback_detection(self, data: Dict[str, Any], source: Optional[str]) -> str:
        ...
        if any(
            self._field_exists_deep(data, field)
            for field in ["companion", "color_identity"]
        ):
            return "mtgo_v2"
    ...
    def normalize_data(
        ...
        try:
            ...
            normalized["schema_detection"] = {
                ...
                "source_hint": source,
                "detection_confidence": self._calculate_schema_score(
                    data, schema_version
                ),
            }
    ...
    def validate_data(
        ...
        try:
            ...
            validation_issues = self._perform_additional_validation(
                data, schema_version
            )
    ...
    def _perform_additional_validation(
        ...
        player_count = (
            ...
            or len(data.get("standings", []))
            or data.get("playerCount", 0)
            or data.get("player_count", 0)
        )
    ...
    def list_schemas(self) -> Dict[str, str]:
        ...
        return {name: schema.get_version() for name, schema in self.schemas.items()}
