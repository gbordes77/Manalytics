...
class TopdeckScraper(BaseScraper):
    ...
    async def authenticate(self):
        ...
        try:
            ...
            api_key_file = self.api_config.get(
        ...
        try:
            ...
            while True:
                ...
                async with self.session.get(
                    ...
                    page_tournament_ids = self.extract_tournament_ids_from_html(
                        html_content
                    )
    ...
    async def fetch_tournament(self, tournament_id: str) -> Optional[Dict]:
        ...
        try:
            ...
            async with self.session.get(url, params=params) as resp:
                if resp.status != 200:
                    self.logger.error(
                        f"Failed to fetch tournament {tournament_id}: {resp.status}"
                    )
                    return None
            ...
            if tournament_data:
                return self.format_tournament_data(
                    tournament_data, tournament_data.get("standings", [])
                )
            else:
                return None
    ...
    def parse_tournament_html(
        self, html_content: str, tournament_id: str
    ) -> Optional[Dict]:
    ...
    def extract_standings(self, soup) -> List[Dict]:
        ...
        try:
            ...
            decklist_links = soup.find_all(
                "a", href=lambda x: x and "deck" in x.lower()
            )
