...
async def get_metagame_data(
    ...
    if format:
        ...
        filtered_decks = [
            deck
            for deck in decks
            if deck.get("tournament_format", "").lower() == format.lower()
        ]
        if filtered_decks:
...
async def generate_analysis(request: Request):
    ...
    try:
        ...
        if not format or not start_date or not end_date:
            raise HTTPException(
                status_code=400, detail="Format, start_date et end_date sont requis"
            )
        ...
        process = await asyncio.create_subprocess_shell(
            command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, cwd=project_dir
        )
...
async def open_finder(request: Request):
    ...
    try:
        ...
        process = await asyncio.create_subprocess_shell(
            f"open '{path}'", stdout=subprocess.PIPE, stderr=subprocess.PIPE
        )
...
if __name__ == "__main__":
    ...
    if data_info.get("data_loaded"):
        print(
            f"✅ Données chargées: {data_info['total_decks']} decks, {data_info['total_tournaments']} tournois"
        )
    else:
