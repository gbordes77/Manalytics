...
class ConnectionManager:
    ...
    async def connect(
        self, websocket: WebSocket, format: str, user_id: Optional[str] = None
    ):
        ...
        logger.info(
            f"Client connecté au format {format}, total: {len(self.active_connections[format])}"
        )
...
class RealtimeMetagameService:
    ...
    async def add_connection(
        ...
        await websocket.send_json(
            {"type": "initial_state", "format": format, "data": current_state}
        )
    ...
    async def tournament_completed(
        self, tournament_id: str, format: str, results: Dict
    ):
        """Traiter la fin d'un tournoi"""
        logger.info(f"Tournoi terminé: {tournament_id} ({format})")

        # Calculer les nouvelles statistiques
        new_stats = await self.calculate_incremental_stats(
            tournament_id, format, results
        )
    ...
    async def meta_shift_detected(self, format: str, shift_data: Dict):
        ...
        update = MetagameUpdate(type="meta_shift", format=format, data=shift_data)
    ...
    async def deck_innovation_detected(self, format: str, innovation_data: Dict):
        ...
        update = MetagameUpdate(
            type="deck_innovation", format=format, data=innovation_data
        )
    ...
    async def calculate_incremental_stats(
        self, tournament_id: str, format: str, results: Dict
    ) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "data_points": current_stats.get("data_points", 0)
                + len(results.get("decks", [])),
            }
    ...
    def _calculate_new_shares(
        self, current_stats: Dict, tournament_impact: Dict
    ) -> Dict:
        """Calculer les nouvelles parts de métagame"""
        ...
        for archetype, current_share in current_shares.items():
            ...
            updated_shares[archetype] = (
                current_weight * current_share + new_weight * new_share
            )
    ...
    async def _update_realtime_metrics(self, update: MetagameUpdate):
        ...
        try:
            ...
            await self.cache.set(
                f"realtime:last_update:{update.format}",
