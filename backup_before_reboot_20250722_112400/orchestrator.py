...
class ManalyticsOrchestrator:
    ...
    async def run_pipeline(self, format: str, start_date: str, end_date: str):
        ...
        try:
            ...
            if len(df) == 0:
                self.logger.warning("⚠️ No data available from MTGODecklistCache")
                # Try to create a minimal DataFrame with required columns
                df = pd.DataFrame(
                    columns=[
                        "player_name",
                        "tournament_id",
            ...
            # Check if DataFrame is not empty and has the required column
            if len(df) > 0 and "tournament_source" in df.columns:
                ...
                self.logger.warning(
                    "⚠️ No data available for filtering - DataFrame is empty or missing tournament_source column"
                )
    ...
    async def generate_visualizations(self, output_dir: str, df: pd.DataFrame):
        ...
        try:
            ...
            if len(df) == 0:
                self.logger.warning(
                    "⚠️ No data available for visualizations - creating empty dashboard"
                )
                # Create a simple empty dashboard
                empty_html = f"""
                <!DOCTYPE html>
                <html>
                <head>
                    <title>No Data Available - {self.format} Analysis</title>
                    <meta charset="utf-8">
                    <style>
                ...
                <body>
                    <div class="container">
                        <h1>📊 {self.format} Analysis</h1>
                        <div class="warning">
                            ⚠️ No tournament data available for the specified period<br>
                            Period: {self.start_date} to {self.end_date}
                        </div>
                        <p>Please check:</p>
                        <ul style="text-align: left; display: inline-block;">
                            <li>MTGODecklistCache submodule is properly initialized</li>
                            <li>Tournament data exists for the specified period</li>
                            <li>Fbettega integration is working correctly</li>
                        </ul>
                    </div>
                </body>
                </html>
                ...
                dashboard_file = (
                    Path(output_dir)
                    / f"{self.format.lower()}_{self.start_date}_{self.end_date}.html"
                )
    ...
    async def _fetch_fbettega_tournaments(self) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            # L'intégrateur Fbettega ne télécharge que les métadonnées des tournois
            # Il faut implémenter le téléchargement des decks séparément
            # Pour l'instant, on retourne None car les tournois n'ont pas de decks

            self.logger.info(
                "📋 Fbettega: Tournament metadata found but no decks downloaded yet"
            )
            self.logger.info(
                "💡 TODO: Implement deck downloading from fbettega tournaments"
            )
            return None
    ...
    def _generate_search_patterns(self):
        ...
        while current_date <= end_date:
            ...
            base_patterns = [
                ...
                f"Analyses/*{self.format.lower()}*/**/*.json",
                # AJOUTER LE PATTERN POUR data/processed/ - CACHE PRINCIPAL
                f"data/processed/*{self.format.lower()}*.json",
                f"data/processed/**/*{self.format.lower()}*.json",
                f"data/processed/*_{self.format.lower()}_*.json",
                f"data/processed/mtgo_{self.format.lower()}_*.json",
                f"data/processed/melee_{self.format.lower()}_*.json",
    ...
    def _process_tournament_file(self, file_path):
        ...
        # Check if this is a file with list of tournaments or list of decks
        if isinstance(tournament_data, list):
            processed_decks = []

            # Check if it's a list of tournaments (like data/processed cache files)
            if (
                len(tournament_data) > 0
                and "tournament" in tournament_data[0]
                and "decks" in tournament_data[0]
            ):
                # This is a list of tournaments with decks (cache format)
                for tournament in tournament_data:
                    tournament_decks = self._process_single_tournament_from_cache(
                        tournament, file_path
                    )
                    processed_decks.extend(tournament_decks)
                return processed_decks

            # This is a list of decks from decklists_detailed.json
            for deck in tournament_data:
                # Extract tournament date from deck data
                tournament_date_str = deck.get("tournament_date", "")
                if not tournament_date_str:
                    continue

                try:
                    tournament_date = datetime.strptime(
                        tournament_date_str, "%Y-%m-%d"
                    ).date()
                except:
                    continue
                ...
                # Check format
                if "standard" not in file_path.lower():
                    continue

                # Process deck
                deck_data = self._process_deck_from_list(
                    deck, tournament_date, file_path
                )
                if deck_data:
        ...
        # Original logic for tournament files
    ...
    def _process_single_tournament_from_cache(self, tournament_data, file_path):
        """Process a single tournament from cache file (data/processed format)"""
        processed_decks = []
        ...
        try:
            ...
            tournament_source = tournament_info.get("Source", "unknown")
            ...
            # Process each deck
            for deck in decks:
                deck_data = self._process_deck_from_cache(
                    deck, tournament_info, tournament_date, file_path
                )
                if deck_data:
                    processed_decks.append(deck_data)
            ...
            self.logger.warning(f"Error processing tournament from cache: {e}")

        return processed_decks

    def _process_deck_from_cache(
        self, deck, tournament_info, tournament_date, file_path
    ):
        """Process deck from cache format (data/processed)"""
        try:
            # Extract basic info
            player_name = deck.get("Player", "Unknown")
            result = deck.get("Result", "0-0")
            ...
            return {
                ...
                "archetype": archetype,
                "archetype_with_colors": archetype_with_colors,
                "color_identity": color_analysis.get("identity", ""),
                "guild_name": color_analysis.get("guild_name", ""),
                "tournament_name": tournament_name,
                "tournament_date": tournament_date,
                "tournament_source": tournament_source,
                "tournament_id": deck_url,  # Use deck URL as unique ID
                "deck_url": deck_url,
                "wins": wins,
                "losses": losses,
                "draws": 0,
            ...
            self.logger.warning(f"Error processing deck from cache: {e}")
            return None

    def _process_deck_from_list(self, deck, tournament_date, file_path):
        """Process deck from decklists_detailed.json format"""
        try:
            # Extract basic info
            player_name = deck.get("player_name", "Unknown")
            archetype = deck.get("archetype", "Unknown")
            archetype_with_colors = deck.get("archetype_with_colors", archetype)
            color_identity = deck.get("color_identity", "")
            guild_name = deck.get("guild_name", "")
            tournament_name = deck.get("tournament_name", "Unknown Tournament")
            tournament_source = deck.get("tournament_source", "unknown")
            ...
            # Calculate winrate
            matches_played = wins + losses
            winrate = wins / max(1, matches_played) if matches_played > 0 else 0
            ...
            return {
                ...
                "tournament_date": tournament_date,
                "tournament_source": tournament_source,
                "tournament_id": deck.get("deck_id", ""),
                "deck_url": deck_url,
                "wins": wins,
                "losses": losses,
                "draws": draws,
            ...
            self.logger.warning(f"Error processing deck from list: {e}")
            return None
    ...
    def generate_dashboard(self, output_dir: str, df: pd.DataFrame):
        ...
        try:
            ...
            # Statistiques générales - INCLURE TOUS LES TOURNOIS
            # CORRECTION: Inclure tous les tournois pour avoir le bon compteur
            filtered_df = df.copy()  # Utiliser toutes les données

            # Compter les tournois uniques par nom ET date
            if "tournament_id" in filtered_df.columns:
                total_tournaments = filtered_df["tournament_id"].nunique()
            elif (
                "tournament_name" in filtered_df.columns
                and "tournament_date" in filtered_df.columns
            ):
                # Compter les combinaisons uniques nom + date pour avoir le vrai nombre de tournois
                total_tournaments = filtered_df.groupby(
                    ["tournament_name", "tournament_date"]
                ).ngroups
            else:
                # Fallback: compter par combinaison date + source
                total_tournaments = filtered_df.groupby(
                    ["tournament_date", "tournament_source"]
                ).ngroups
...
<body>
    ...
    def _generate_leagues_html_template(
        ...
        elif "tournament_name" in df.columns and "tournament_date" in df.columns:
            # Compter les combinaisons uniques nom + date pour avoir le vrai nombre de tournois
            total_tournaments = df.groupby(
                ["tournament_name", "tournament_date"]
            ).ngroups
        else:
            # Fallback: compter par combinaison date + source
