#!/usr/bin/env python3
"""
Melee Tournament Scraper Script - Robust Version
Scrapes Melee.gg tournaments with enhanced features
"""
...
from scrapers.melee_scraper_robust import (
    MeleeScraperRobust, Tournament, Decklist, DeckCard,
    MeleeSettings
)
...
class MeleeDataSaver:
    """Handles saving Melee tournament data"""
    ...
    def save_tournament(self, tournament: Tournament) -> bool:
        ...
        try:
            ...
            if tournament.tournament_type in ["rcq", "championship", "qualifier"]:
    ...
    def _convert_to_dict(self, tournament: Tournament) -> Dict[str, Any]:
...
class ScrapeReport:
    ...
    def generate_summary_report(self, tournaments: List[Tournament], 
        ...
        with open(report_file, 'w') as f:
            f.write("Melee.gg Scraping Report\n")
            f.write("=" * 50 + "\n\n")
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Scrape Melee.gg tournaments with enhanced features",
...
Examples:
  ...
  # Scrape only Standard RCQs
  %(prog)s --format standard --tournament-types rcq
  
  # Scrape multiple formats
  %(prog)s --formats modern,legacy,pioneer --days 7
    ...
    async with MeleeScraperRobust(creds['email'], creds['password']) as scraper:
        for fmt in formats:
            ...
            if args.tournament_types:
                allowed_types = {t.strip().lower() for t in args.tournament_types.split(',')}
                tournaments = [t for t in tournaments if t.tournament_type in allowed_types]
