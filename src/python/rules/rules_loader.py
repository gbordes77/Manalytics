...
class RulesLoader:
    ...
    def load_format_rules(self, format_dir: str) -> Dict[str, Any]:
        ...
        for file_path in Path(format_dir).glob('**/*'):
            if (file_path.is_file() and 
                ...
                file_path.name not in archetype_files):
                ...
                try:
                    ...
                    elif isinstance(additional_rules, dict) and 'archetypes' in additional_rules:
                        rules['archetypes'].extend(additional_rules['archetypes'])
                        
                    logger.debug(f"Loaded additional rules from {file_path}")
                    
                except Exception as e:
                    logger.warning(f"Failed to load additional rules from {file_path}: {e}")
            ...
            if os.path.isdir(format_path) and not item.startswith('.'):
                try:
                    ...
                    if format_rules['archetypes']:  # Only include if has archetypes
                        ...
                        logger.info(f"Loaded {len(format_rules['archetypes'])} archetypes for {item}")
        ...
        if 'conditions' in archetype:
            if not isinstance(archetype['conditions'], list):
                ...
                for j, condition in enumerate(archetype['conditions']):
                    condition_errors = self._validate_condition(condition, f"{prefix}, condition {j}")
    ...
    def _validate_condition(self, condition: Dict[str, Any], prefix: str) -> List[str]:
        ...
        if 'type' in condition:
            if condition['type'] not in valid_types:
                errors.append(f"{prefix}: invalid type '{condition['type']}', must be one of {valid_types}")
    ...
    def get_format_from_path(self, file_path: str) -> Optional[str]:
        ...
        for parent in path.parents:
            ...
            if parent_name in ['modern', 'legacy', 'vintage', 'standard', 'pioneer', 'historic']:
                return parent_name
        
        # Try to extract from filename
        filename = path.stem.lower()
        if any(fmt in filename for fmt in ['modern', 'legacy', 'vintage', 'standard', 'pioneer', 'historic']):
            for fmt in ['modern', 'legacy', 'vintage', 'standard', 'pioneer', 'historic']:
                if fmt in filename:
                    return fmt
