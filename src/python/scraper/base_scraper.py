...
class BaseScraper(ABC):
    ...
    async def fetch_date_range(self, format_name: str, start_date: str, end_date: str) -> List[Dict]:
        ...
        self.logger.info(f"Scraping {format_name} tournaments from {start_date} to {end_date}")
        
        async with self:
            # Rechercher les tournois
            tournament_ids = await self.search_tournaments(format_name, start_date, end_date)
    ...
    async def _fetch_tournament_with_semaphore(self, semaphore: asyncio.Semaphore, tournament_id: str) -> Optional[Dict]:
        ...
        async with semaphore:
            ...
            for attempt in range(self.max_retries):
                try:
                    ...
                    self.logger.warning(f"Attempt {attempt + 1} failed for tournament {tournament_id}: {e}")
                    if attempt < self.max_retries - 1:
                        # Exponential backoff
                        delay = self.retry_delay * (2 ** attempt) + random.uniform(0, 1)
                        await asyncio.sleep(delay)
                        
            self.logger.error(f"Failed to fetch tournament {tournament_id} after {self.max_retries} attempts")
            return None
    ...
    async def save_tournament(self, tournament: Dict) -> None:
        ...
        try:
            ...
            path = Path(self.cache_folder) / "raw" / source_name / str(year) / f"{month:02d}" / f"{day:02d}"
