# -*- coding: utf-8 -*-
"""
Copie directe du code de fbettega/mtg_decklist_scrapper/Client/MTGOclient.py
...
class TournamentList:
    ...
    def DL_tournaments(start_date: datetime, end_date: datetime = None) -> List[dict]:
        ...
        while current_date <= end_date:
            ...
            if response.status_code != 200:
                current_date = TournamentList.increment_month(
                    current_date
    ...
    def get_tournament_details(self, tournament: Tournament) -> "CacheItem":
        ...
        json_data = data_row[
            29:-1
        ]  # Skip 29 caractères initiaux et retirer le dernier caractère (point-virgule)
        ...
        winloss = (
            TournamentLoader.parse_winloss(event_json)
            if event_type == "tournament"
            else None
        )
        standings = (
            TournamentLoader.parse_standing(event_json, winloss)
            if event_type == "tournament"
            else None
        )
        bracket = (
            TournamentLoader.parse_bracket(event_json)
            if event_type == "tournament"
            else None
        )
        # Parser les decks
        decks = TournamentLoader.parse_decks(
...
class TournamentLoader:
    ...
    def parse_decks(tournament, event_type, winloss, event_json):
        ...
        for deck in event_json["decklists"]:
            ...
            deck_ite = DeckNormalizer.normalize(
                Deck(
                    date=event_date,
                    player=player,
                    result=result,
...
# Classe OrderNormalizer simulée
class OrderNormalizer:
    @staticmethod
    def reorder_decks(decks, standings, bracket, has_bracket):
        # Pour l'instant, retourner les decks tels quels
