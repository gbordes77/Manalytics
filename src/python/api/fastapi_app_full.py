...
async def get_metagame(format: str, date: Optional[str] = None):
    """Get metagame for a format with real data"""
    try:
        # Load real data
        real_data_path = PROJECT_ROOT / "real_data" / "complete_dataset.json"
        ...
        if not format_tournaments:
            raise HTTPException(
                status_code=404, detail=f"Aucun tournoi trouvé pour {format}"
            )
...
async def get_archetype_details(
    ...
    """Archetype details with real data"""
    try:
        # Use engine to analyze archetype
...
async def get_recent_tournaments(limit: int = 10):
    """Get recent tournaments with real data"""
    try:
        real_data_path = PROJECT_ROOT / "real_data" / "complete_dataset.json"
        ...
        recent_tournaments = sorted(
            tournaments, key=lambda t: t.get("tournament_date", ""), reverse=True
        )[:limit]
...
async def global_exception_handler(request, exc):
    ...
    return HTTPException(
        status_code=500, detail=f"Erreur interne du serveur: {str(exc)}"
    )
