...
class DataQuarantine:
    ...
    def _validate_data_quality(self, tournament: Dict[str, Any]) -> List[str]:
        ...
        if player_count < self.thresholds['min_players']:
            issues.append(f"Too few players: {player_count} < {self.thresholds['min_players']}")
        elif player_count > self.thresholds['max_players']:
            issues.append(f"Too many players: {player_count} > {self.thresholds['max_players']}")
        
        # Unknown archetype rate
        unknown_rate = self.calculate_unknown_rate(tournament)
        if unknown_rate > self.thresholds['max_unknown_rate']:
            issues.append(f"High unknown archetype rate: {unknown_rate:.1%} > {self.thresholds['max_unknown_rate']:.1%}")
    ...
    def _analyze_win_rate_distribution(self, win_rates: List[float]) -> List[str]:
        ...
        try:
            ...
            outliers = [wr for wr in win_rates 
                       if abs(wr - mean_wr) > self.statistical_thresholds['win_rate_stddev_threshold'] * stdev_wr]
            
            if len(outliers) > len(win_rates) * 0.1:  # More than 10% outliers
                issues.append(f"Unusual win rate distribution: {len(outliers)} outliers")
            
            # Check for impossible win rates
            impossible_rates = [wr for wr in win_rates if wr < 0 or wr > 1]
            if impossible_rates:
                issues.append(f"Impossible win rates detected: {len(impossible_rates)} cases")
    ...
    def _analyze_archetype_distribution(self, players: List[Dict[str, Any]]) -> List[str]:
        ...
        for archetype, count in archetype_counts.items():
            ...
            if frequency > 0.8:  # 80% of field
                issues.append(f"Archetype '{archetype}' dominates field: {frequency:.1%}")
    ...
    def _analyze_performance_consistency(self, players: List[Dict[str, Any]]) -> List[str]:
        ...
        for pattern, count in performance_patterns.items():
            if count > len(players) * 0.3:  # More than 30% have same record
                issues.append(f"Suspicious performance pattern: {count} players with {pattern} record")
    ...
    def has_missing_standings(self, tournament: Dict[str, Any]) -> bool:
        ...
        missing_count = 0
        for player in players:
            if not player.get('rank') or (player.get('wins') is None and player.get('losses') is None):
                missing_count += 1
        ...
        for i, player in enumerate(players):
            ...
            if sideboard:
                ...
                if side_size > self.thresholds['max_sideboard_size']:
                    issues.append(f"Player {i+1}: sideboard too large ({side_size} cards)")
    ...
    def _log_quarantine(self, tournament: Dict[str, Any], issues: List[str]):
        ...
        logger.warning(f"Tournament quarantined: {tournament.get('tournament_id')} - {len(issues)} issues")
    ...
    def get_quarantine_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'quarantine_rate': len(self.quarantine_history),  # Would need total processed
