...
class AdvancedArchetypeClassifier:
    ...
    def __init__(self):
        ...
        self.archetype_patterns = {
            'Control': {
                'keywords': ['control', 'counterspell', 'wrath', 'board clear', 'card draw'],
                'card_indicators': ['Supreme Verdict', 'Counterspell', 'Teferi', 'Jace', 'Wrath of God'],
                'strategy': 'control'
            },
            'Aggro': {
                'keywords': ['aggressive', 'burn', 'rush', 'fast', 'cheap creatures'],
                'card_indicators': ['Lightning Bolt', 'Goblin Guide', 'Monastery Swiftspear'],
                'strategy': 'aggro'
            },
            'Midrange': {
            ...
            'Ramp': {
                ...
                'card_indicators': ['Rampant Growth', 'Llanowar Elves', 'Primeval Titan'],
                'strategy': 'ramp'
            }
        }
        ...
        self.enhanced_archetypes = {
            'Prowess': {
                ...
                'key_cards': ['Monastery Swiftspear', 'Soul-Scar Mage', 'Stormwing Entity'],
                'strategy_type': 'aggro'
            },
            'Burn': {
                'base_name': 'Burn',
    ...
    def classify_with_color_integration(self, deck_data: Dict) -> Dict[str, str]:
        ...
        for archetype, score in synergy_scores.items():
            if score > best_score and score > 0.5:  # Lowered threshold for better matching
                best_score = score
    ...
    def generate_archetype_statistics(self, classified_decks: List[Dict]) -> Dict:
        ...
        stats['classification_accuracy'] = 1.0 - (others_count / max(len(classified_decks), 1))
        stats['others_percentage'] = (others_count / max(len(classified_decks), 1)) * 100
    ...
    def improve_classification_rules(self, feedback_data: List[Dict]) -> None:
        ...
        for archetype, card_lists in archetype_corrections.items():
            ...
            if archetype not in self.enhanced_archetypes:
                ...
                self.enhanced_archetypes[archetype]['key_cards'] = list(existing_cards.union(new_cards))[:10] 