...
class BusinessMetrics:
    ...
    def calculate_classification_metrics(self, start_date: datetime, end_date: datetime) -> Dict[str, Any]:
        ...
        try:
            ...
            classified_decks = sum(1 for entry in data if entry.get('archetype') != 'Unknown')
            unknown_rate = (total_decks - classified_decks) / total_decks if total_decks > 0 else 0
            
            classification_metrics = {
                'total_decks_processed': total_decks,
                'successfully_classified': classified_decks,
                'classification_rate': classified_decks / total_decks if total_decks > 0 else 0,
    ...
    def calculate_data_quality_metrics(self, start_date: datetime, end_date: datetime) -> Dict[str, Any]:
        ...
        try:
            ...
            complete_entries = sum(1 for entry in data if all(
                entry.get(field) for field in ['player_name', 'archetype', 'tournament_date']
            ))
            
            quality_metrics = {
                'overall_quality_score': complete_entries / total_entries if total_entries > 0 else 0,
                'completeness_score': complete_entries / total_entries if total_entries > 0 else 0,
