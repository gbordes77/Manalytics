...
class MetagameChartsGenerator:
    ...
    def __init__(self, data_path: str = None):
        ...
        self.archetype_colors = [
            '#762a83', '#1b7837', '#d6604d', '#4393c3', '#f4a582',
            '#92c5de', '#d1e5f0', '#fddbc7', '#b2182b', '#2166ac',
            '#5aae61', '#9970ab', '#c2a5cf', '#a6dba0', '#008837',
            '#7b3294', '#c51b7d', '#de77ae', '#f1b6da', '#fde0ef',
    ...
    def create_metagame_pie_chart(self, df: pd.DataFrame, min_threshold: float = 0.01) -> go.Figure:
        ...
        small_archetypes_sum = archetype_shares[archetype_shares < min_threshold * 100].sum()
        ...
        elif main_archetypes.index[0] == 'Autres / Non classifiés' and len(main_archetypes) > 1:
            # Réorganiser pour que "Autres" soit en dernier
    ...
    def calculate_archetype_stats(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        stats_df = df.groupby('archetype').agg({
        ...
        stats_df.columns = [
            'winrate_mean', 'winrate_std', 'sample_size',
            'total_wins', 'total_losses', 'total_matches', 'unique_players'
        ]
        ...
        stats_df['metagame_share'] = stats_df['unique_players'] / stats_df['unique_players'].sum()
    ...
    def create_metagame_share_chart(self, stats_df: pd.DataFrame, threshold: float = 0.05) -> go.Figure:
        ...
        fig.add_trace(go.Bar(
            ...
            customdata=list(zip(
                filtered_df['unique_players'],
                filtered_df['winrate_mean']
            ))
        ))
    ...
    def create_tiers_scatter_plot(self, stats_df: pd.DataFrame) -> go.Figure:
        ...
        for tier in stats_df['tier'].unique():
            ...
            fig.add_trace(go.Scatter(
                ...
                customdata=list(zip(
                    tier_data['unique_players'],
                    tier_data['winrate_mean']
                ))
            ))
    ...
    def create_bubble_chart_winrate_presence(self, stats_df: pd.DataFrame) -> go.Figure:
        ...
        for tier in stats_df['tier'].unique():
            ...
            fig.add_trace(go.Scatter(
                ...
                mode='markers+text',
                marker=dict(
                    size=tier_data['unique_players'] * 2,  # Taille = nombre de joueurs
                    color=tier_colors.get(tier, 'gray'),
                    line=dict(width=2, color='white'),
                    opacity=0.7
                ),
    ...
    def create_archetype_evolution_chart(self, data):
        ...
        daily_counts = df.groupby(['date', 'archetype']).size().reset_index(name='count')
        
        # Créer un pivot pour avoir les archétypes en colonnes
        pivot_data = daily_counts.pivot(index='date', columns='archetype', values='count').fillna(0)
        
        # Trier les archétypes par popularité totale
        archetype_totals = df['archetype'].value_counts()
        top_archetypes = archetype_totals.head(5).index.tolist()  # Limiter à 5 pour la lisibilité
    ...
    def create_main_archetypes_bar_chart(self, data):
        ...
        others_percentage = archetype_percentages.iloc[15:].sum() if len(archetype_percentages) > 15 else 0
    ...
    def export_all_data(self, stats_df: pd.DataFrame, df: pd.DataFrame, output_dir: str = "analysis_output"):
    ...
    def generate_all_charts(self, df: pd.DataFrame, output_dir: str = "analysis_output") -> Dict:
        ...
        charts = {
            ...
            'bubble_winrate_presence': self.create_bubble_chart_winrate_presence(stats_df),
            'top_5_0': self.create_top_5_0_chart(df),
            'data_sources_pie': self.create_data_sources_pie_chart(df.to_dict('records')), # NOUVEAU !
            'archetype_evolution': self.create_archetype_evolution_chart(df.to_dict('records')), # NOUVEAU !
            'main_archetypes_bar': self.create_main_archetypes_bar_chart(df.to_dict('records')) # NOUVEAU !
        }
