...
class MTGODecklistCacheFormatter:
    ...
    def format_tournament_file(self, tournament_file: Path, format_name: str) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Failed to format tournament file {tournament_file}: {e}")
            return False
    ...
    def format_tournament_data(self, raw_data: Dict, format_name: str) -> Optional[Dict]:
        ...
        try:
            ...
            formatted = {
                "Tournament": {
                    ...
                    "Type": self.determine_tournament_type(tournament_info.get('name', '')),
                    "Source": "melee.gg",
        ...
        try:
            ...
            formatted = {
                "Tournament": {
                    ...
                    "Name": tournament_info.get('name', tournament_info.get('Name', '')),
                    "Date": tournament_info.get('date', tournament_info.get('Date', datetime.now().isoformat())),
                    "Format": tournament_info.get('format', tournament_info.get('Format', format_name)),
                    "Players": len(data.get('decks', data.get('Standings', []))),
                    "Rounds": tournament_info.get('rounds', tournament_info.get('Rounds', 0)),
                    "Type": tournament_info.get('type', tournament_info.get('Type', 'Tournament')),
                    "Source": tournament_info.get('source', tournament_info.get('Source', 'unknown')),
