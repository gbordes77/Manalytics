...
class MeleeScraper(BaseScraper):
    ...
    async def scrape_tournaments(self, start_date: datetime, end_date: datetime) -> List[Dict[str, Any]]:
        ...
        try:
            # POST request to Tournament SearchResults
            ...
            for tournament in tournaments_data:
                # Check format - try different key names
                tournament_format = tournament.get('gameDescription', '').lower()
                if not tournament_format:
                    tournament_format = tournament.get('Format', '').lower()
                
                # Skip if not Magic
                if 'magic' not in tournament_format.lower():
                    continue
                
                tournament_id = tournament.get('id') or tournament.get('ID')
                ...
                tournament_info = {
                    ...
                    "name": tournament.get('name') or tournament.get('Name', 'Unknown Tournament'),
                    "date": self._parse_date(tournament.get('startDate') or tournament.get('StartDate', '')),
