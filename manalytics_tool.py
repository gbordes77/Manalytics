...
class ManalyticsEngine:
    ...
    def __init__(self, format_name: str, start_date: str, end_date: str, output_dir: str = None):
        self.format_name = format_name.lower()
        self.start_date = datetime.strptime(start_date, '%Y-%m-%d').date()
        self.end_date = datetime.strptime(end_date, '%Y-%m-%d').date()
        self.output_dir = output_dir or f"{self.format_name}_analysis_{start_date}_{end_date}"
        self.df = None
    ...
    def load_tournament_data(self):
        ...
        if not all_decks:
            print(f"❌ Aucun deck trouvé pour {self.format_name} dans la période spécifiée")
            return False
        ...
        print(f"📅 Période réelle: {self.df['tournament_date'].min().strftime('%Y-%m-%d')} à {self.df['tournament_date'].max().strftime('%Y-%m-%d')}")
    ...
    def _process_tournament_file(self, file_path: str):
        ...
        if processed_decks:
            ...
            print(f"✅ {tournament_name} ({tournament_date}) - {len(processed_decks)} decks")
    ...
    def _classify_standard_archetype(self, mainboard):
        ...
        archetype_cards = {
            "Jeskai Control": ["Teferi, Hero of Dominaria", "Teferi, Time Raveler", "Dovin's Veto", "Wrath of God", "Day of Judgment"],
            "Bant Ramp": ["Overlord of the Hauntwoods", "Beza, the Bounding Spring", "Leyline Binding"],
            "Jeskai Convoke": ["Helping Hand", "Proft's Eidetic Memory", "Steamcore Scholar"],
            "Mono Red Aggro": ["Lightning Bolt", "Goblin Guide", "Monastery Swiftspear", "Torch the Tower"],
            "Azorius Control": ["Teferi, Hero of Dominaria", "Absorb", "Dovin's Veto", "Wrath of God"],
        }
        ...
        if matched_archetype and any(mono in matched_archetype for mono in ["Mono Blue", "Mono White", "Mono Black", "Mono Green"]):
            return "Other"
    ...
    def _classify_modern_archetype(self, mainboard):
        ...
        archetype_cards = {
            "Burn": ["Lightning Bolt", "Lava Spike", "Rift Bolt", "Monastery Swiftspear"],
            "Affinity": ["Ornithopter", "Mox Opal", "Arcbound Ravager", "Steel Overseer"],
            "Tron": ["Karn Liberated", "Ugin, the Spirit Dragon", "Urza's Tower", "Urza's Mine"],
            "Death's Shadow": ["Death's Shadow", "Thoughtseize", "Street Wraith", "Temur Battle Rage"],
        ...
        if matched_archetype and any(mono in matched_archetype for mono in ["Mono Red", "Mono Blue", "Mono White", "Mono Black", "Mono Green"]):
            return "Other"
    ...
    def _classify_legacy_archetype(self, mainboard):
        ...
        archetype_cards = {
            ...
            "Miracles": ["Terminus", "Counterbalance", "Sensei's Divining Top", "Brainstorm"],
            "Storm": ["Tendrils of Agony", "Lion's Eye Diamond", "Infernal Tutor", "Dark Ritual"],
    ...
    def _detect_colors(self, card_names):
        ...
        color_indicators = {
            'W': ['Plains', 'Wrath', 'Teferi', 'Dovin', 'Day of Judgment', 'Swords to Plowshares'],
            'U': ['Island', 'Counterspell', 'Teferi', 'Dovin', 'Force of Will', 'Brainstorm'],
            'B': ['Swamp', 'Thoughtseize', 'Sheoldred', 'Liliana', 'Dark Ritual', 'Fatal Push'],
        ...
        return color_combinations.get(color_set, 'Multicolor' if len(detected_colors) > 3 else 'Unknown')
    ...
    def analyze_metagame(self):
        ...
        print("-" * 30)
        archetype_winrates = self.df.groupby('archetype').agg({
            'wins': 'sum',
            'losses': 'sum',
            'winrate': 'mean',
            'player_name': 'count'
        ...
        archetype_winrates = archetype_winrates.sort_values('global_winrate', ascending=False)
        
        for archetype, stats in archetype_winrates.head(10).iterrows():
            print(f"🎯 {archetype}: {stats['global_winrate']:.3f} ({stats['player_name']} decks)")
    ...
    def _create_winrates_bar_chart(self, output_path):
        ...
        archetype_stats.columns = ['total_wins', 'total_losses', 'avg_winrate', 'winrate_std', 'deck_count']
    ...
    def _create_source_analysis(self, output_path):
        ...
        fig.add_trace(go.Bar(x=source_stats.index, y=source_stats['tournament_id']), row=1, col=1)
        fig.add_trace(go.Bar(x=source_stats.index, y=source_stats['player_name']), row=1, col=2)
        fig.add_trace(go.Bar(x=source_stats.index, y=source_stats['winrate']), row=1, col=3)
    ...
    def _export_data(self, output_path):
        ...
        # JSON
        self.df.to_json(output_path / "donnees_analysees.json", orient='records', indent=2)
