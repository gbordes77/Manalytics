#!/usr/bin/env python3
"""
Create MODERN dashboard with exact template from the reference.
Uses MTGOData with proper match-based calculations.
"""
...
class ModernDashboardCreator:
    """Creates modern dashboard with exact template styling"""
    ...
    def create_modern_dashboard(self, start_date: datetime, end_date: datetime):
        """Create modern dashboard with exact template"""
        print(f"🎨 Creating modern dashboard: {start_date.date()} to {end_date.date()}")
        ...
        print("\n🎨 Creating modern visualization...")
        html = self._create_modern_html(meta_data, timeline_data)
        
        output_path = Path('data/cache/mtgodata_modern_dashboard.html')
    ...
    def _create_modern_html(self, meta_data: Dict, timeline_data: Dict) -> str:
        """Create modern HTML with exact template styling"""
        # Get date range
        start_date = datetime(2025, 7, 1).strftime('%Y-%m-%d')
        end_date = datetime(2025, 7, 21).strftime('%Y-%m-%d')
        
        # Prepare data for charts
        meta_breakdown = meta_data['meta_breakdown'][:9]  # Top 9 for pie chart
        
        # Create main figure
        mainFig = self._create_main_figure(meta_breakdown, timeline_data)
        
        # Create table figure
        tableFig = self._create_table_figure(meta_breakdown)
        
        # Generate gradient info
        gradient_info = []
        for i, item in enumerate(meta_breakdown):
            colors = get_archetype_colors(item['archetype'])
            if len(colors) > 1:
                gradient_info.append({
                    'index': i,
                    'label': item['archetype'],
                    'colors': colors,
                    'hex_colors': [MTG_COLORS.get(c, '#808080') for c in colors]
                })
...
def main():
    """Main function"""
    creator = ModernDashboardCreator()
    
    # Analyze July 1-21, 2025
    start_date = datetime(2025, 7, 1)
    end_date = datetime(2025, 7, 21)
    
    creator.create_modern_dashboard(start_date, end_date)
